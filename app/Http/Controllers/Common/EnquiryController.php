...
class EnquiryController extends Controller
{
    ...
    public function contact_all(Request $request)
    {
        try {
            ...
            $sortColumn = $request->has("sort_column")
        ...
        try {
            ...
            foreach ($enquiries as $index => $enquiry) {
                $csvData[] = [
                    ...
                    Carbon::parse($enquiry->created_at)
                        ->timezone("Asia/Kolkata")
                        ->format("Y-m-d h:i:s A"), // Created At
                ];
            }
            ...
            // dd($e->getMessage());
            return view(
                "admin.common.enquires-download",
                compact("title", "heading"),
            );
        }
    }
    ...
    public function downloadSubscribe(Request $request)
    {
        try {
            ...
            $fileName =
                "subscribe_list_" . now()->format("Y-m-d_H-i-s") . ".csv";
            $fileContents = "";
            foreach ($csvData as $value) {
                $fileContents .=
