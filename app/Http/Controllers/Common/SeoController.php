...
{
    /**
     * Display SEO entries in admin panel
     */
    ...
    /**
     * Fetch SEO entries for DataTables (AJAX)
     */
    public function allSeo(Request $request)
    {
        try {
            ...
            $seos = $query->orderBy($sortColumn, $sortDirection)
                ->offset($offset)
                ->limit($perPage)
                ->get();
    ...
    {
        $page = collect($this->getPages())->firstWhere('url', $url);
        return $page['title'] ?? null;
    }

    /**
     * Show the form to create new SEO entry
     */
    public function create()
    {
        ...
        foreach ($pages as &$page) {
            $page['disabled'] = in_array($page['url'], $existingPages);
        }
    ...
    /**
     * Show the form to edit an existing SEO entry
     */
    public function seo_edit(Seo $seo)
    {
        $pages = $this->getPages();
        $existingPages = Seo::where('id', '!=', $seo->id)->pluck('page_url')->toArray();
    ...
    public function delete(Request $request)
    {
        ...
        try {
            ...
            // Update deleted_by before deleting (optional, if you track deletions)
            if (auth()->check()) {
                $seo->deleted_by = auth()->id(); // Make sure `deleted_by` column exists
                $seo->save();
            }
    ...
    /**
     * Display SEO entries on website (frontend)
     */
    public function seo_display(Request $request)
    {
        ...
        $total = Seo::where('status', 1)->count();
