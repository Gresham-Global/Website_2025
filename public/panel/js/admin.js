...
$(document).ready(function () {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        var editBlogUrl = baseUrl + '/admin/media/edit/' + row.id;
                        var buttonsHtml =
                            ...
                            '<a href="' + editBlogUrl + '" class="btn btn-primary btn-sm mr-1 mb-1"><span class="material-symbols-outlined">edit</span></a>' +
                            '<a href="javascript:void(0)" onclick="deleteMediaRow(' + row.id + ',\'' + row.title + '\')"  class="btn btn-danger btn-sm delete btn-sm btn btn-danger"> <span class="material-symbols-outlined">delete</span> </a>';
        ...
        table = $('#event-table').DataTable({
            ...
            columns: [{
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        var editBlogUrl = baseUrl + '/admin/event/edit/' + row.id;
                        return `
    ...
    if ($('#add-event').length) {
        ...
        jQuery.validator.addMethod("noRandomNumbers", function (value, element) {
            ...
            return !hasStandaloneNumbers || /\d{4}/.test(value) || /\b\d{1,2}\b/.test(value);
        ...
        jQuery.validator.addMethod("maxLimit", function (value, element) {
            if (this.optional(element)) return true;
        ...
        $("#add-event").validate({
            ...
            errorPlacement: function (error, element) {
                const errorId = element.attr("name") + "-error";
                if ($("#" + errorId).length) {
                    $("#" + errorId).html(error.text()).addClass("show-error");
                } else {
                    error.insertAfter(element);
                }
            },
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return dateObj.toLocaleString('en-IN', options).replace('GMT+5:30', 'IST');
                    }
                },
                {
                    data: 'share_link',
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        var editBlogUrl = baseUrl + '/admin/newsandblog/edit/' + row.id;
                        var buttonsHtml =
                            '<a href="' + editBlogUrl + '" class="btn btn-primary btn-sm mr-1 mb-1"><span class="material-symbols-outlined">edit</span></a>' +
                            '<a href="javascript:void(0)" onclick="deleteNewsRow(' + row.id + ',\'' + row.title + '\')" class="btn btn-danger btn-sm"> <span class="material-symbols-outlined">delete</span> </a>';
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function (tags, type, row) {
                        if (!Array.isArray(tags) || tags.length === 0) return '<span>No Tags</span>';
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        var editBlogUrl = baseUrl + '/admin/publication/edit/' + row.id;
                        var buttonsHtml =
                            '<a href="' + editBlogUrl + '" class="btn btn-primary btn-sm mr-1 mb-1"><span class="material-symbols-outlined">edit</span></a>' +
                            '<a href="javascript:void(0)" onclick="deletePublicationRow(' + row.id + ',\'' + row.title + '\')" class="btn btn-danger btn-sm"> <span class="material-symbols-outlined">delete</span> </a>';
    ...
    if ($('#add-publications').length) {
        ...
        $.validator.addMethod("acceptPdf", function (value, element, param) {
            console.log("accept method called with value: " + value + " and param: " + param);
        ...
        $.validator.addMethod("maxFileSize", function (value, element, param) {
            ...
            console.log(`Validating file size: ${fileSizeBytes} bytes, ${fileSizeKB} KB, ${fileSizeMB} MB`);
        ...
        $("#add-publications").validate({
            ...
            messages: {
                ...
                "report_title[0]": {
                    required: "Please enter at least one title for Report List 1."
                },
                "report_image[0]": {
                    required: "Please enter at least one image for Report List 1."
                },
                "report_list[0][]": {
                    required: "Please enter at least one list for Report List 1."
                },
                "highlight_icon[0]": {
            ...
            errorPlacement: function (error, element) {
                ...
                const errorId = elementId ? elementId + '-error' : element.attr("name") + '-error';
            ...
            highlight: function (element) {
                ...
                const errorContainer = $("#" + name.replace(/\[\]/g, "\\[\\]") + "-error");
            ...
            unhighlight: function (element) {
                ...
                if (errorContainer.length > 0) {
                    errorContainer.text('').removeClass("show-error showerror").addClass("hide-error");
                } else {
                    $(element).siblings('.error').text('').removeClass("show-error showerror").addClass("hide-error");
                }
            },
            submitHandler: function (form) {
                var flag = 0;
                ...
                ["title", "short_description", "description", "thumbnail_image", "tags", "share_link", "report_list[0][]", "highlight_icon[0]", "highlight_description-error-0"].forEach(function (field) {
                    const errorContainer = `#${field}-error`;
                    $(errorContainer).text('').removeClass("show-error showerror").addClass("hide-error");
                });

                // Check Summernote content specifically for #description
                let summernoteVal = $.trim($('#description').summernote('code'));
                ...
                for (let i = 0; i < 3; i++) {
                    let reportListInput = $(`input[name="report_list[${i}][]"]`);
                    let reportListValue = $.trim(reportListInput.val());

                    if (!reportListValue) {
                        $(`#report_list[${i}][]-error`)
                            .text(`Please enter at least one item for Report List ${i + 1}.`)
                            .removeClass("hide-error")
                            .addClass("show-error showerror");
                        flag = 1;
                    } else {
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    let highlightDescriptionVal = $.trim(highlightDescription.val());

                    if (highlightDescriptionVal === '') {
                        $(`#highlight_description_${i}-error`)
                            .text(`Please provide a description for Highlight ${i + 1}.`)
                            .removeClass("hide-error")
                            .addClass("show-error showerror");
                        flag = 1;
                    } else {
        ...
        table = $('#cities-table').DataTable({
            ...
            columns: [{
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        var editCityUrl = baseUrl + '/admin/event/cities/edit/' + row.event_id;
                        var buttonsHtml =
                            '<a href="' + editCityUrl + '" class="btn btn-primary btn-sm mr-1 mb-1"><span class="material-symbols-outlined">edit</span></a>'; // +
            ...
            order: [
                [0, 'desc']
            ], // Default sorting by news_id in descending order
        });
    }
    ...
    if ($('#add-city').length) {
        ...
        $("#add-city").validate({
            ...
            unhighlight: function (element) {
                ...
                if (typeof name !== "undefined") {
                    name = name.replace(/\[\]$/, "");
                    $("#" + name + "-error").removeClass("show-error").empty().hide();
                }
            },
            submitHandler: function (form) {
                form.submit();
    ...
    if ($('#add-city-image').length) {
        ...
        $('#add-image-btn').on('click', function () {
            ...
            newImageDiv.find('input[type="file"]').attr('accept', 'image/png, image/jpeg, image/jpg, image/webp');
        ...
        $("#add-city-image").validate({
            ...
            unhighlight: function (element) {
                const name = $(element).attr("name").replace("[]", "");
                $("#" + name + "-error").removeClass("show-error").empty();
            },
            submitHandler: function (form) {
    ...
    if ($('#edit-city-image').length) {
        ...
        $('#image-upload-container').on('click', '.remove-new-image', function () {
        ...
        table = $('#event-city-image-table').DataTable({
            ...
            columns: [{
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        var editCityUrl = baseUrl + '/admin/event/city/images/edit/' + row.event_id + '/' + row.city_id;
                        var buttonsHtml =
                            '<a href="' + editCityUrl + '" class="btn btn-primary btn-sm mr-1 mb-1"><span class="material-symbols-outlined">edit</span></a>'; // +
    ...
    if ($('#add-career').length) {
        ...
        $.validator.addMethod('validWebp', function (value, element) {
            ...
            return this.optional(element) || (element.files[0] && allowedTypes.includes(element.files[0].type));
        }, 'Please upload a valid image (webp, png, jpg, jpeg).');
        $.validator.addMethod('checkFileSize', function (value, element, param) {
            // Check if file exists and its size is less than or equal to maxSize
            return this.optional(element) || (element.files[0] && element.files[0].size <= 1024 * 1024);
        });
        ...
        $("#add-career").validate({
            ...
            messages: {
                ...
                cover_image: {
                    required: "Select the thumbnail image",
                    validWebp: "Please upload a valid image (webp, png, jpg, jpeg).",
            ...
            submitHandler: function (form) {
                ...
                if ($.trim($('#short_description').summernote('code')) === '') {
                    $("#short_description_error").text("short description cannot be empty.").css("display", "block");
                    return false;
                }
            ...
            columns: [{
...

                {
                    ...
                    render: function (data, type, row) {
                        ...
                        var editBlogUrl = baseUrl + '/admin/career/edit/' + row.id;
                        var buttonsHtml =
                            // '<a href="' + viewBlogUrl + '" class="btn btn-warning btn-sm mr-1 mb-1" target="_blank"><span class="material-symbols-outlined">visibility</span></a>' +
                            '<a href="' + editBlogUrl + '" class="btn btn-primary btn-sm mr-1 mb-1"><span class="material-symbols-outlined">edit</span></a>' +
                            '<a href="javascript:void(0)" onclick="deleteCareerRow(' + row.id + ',\'' + row.title + '\')"  class="btn btn-danger btn-sm delete btn-sm btn btn-danger"> <span class="material-symbols-outlined">delete</span> </a>';
    ...
    if ($('#job-table').length) {
            ...
            columns: [{
...

                {
                    ...
                    render: function (data, type, row) {
                        ...
                        var viewBlogUrl = baseUrl + '/admin/job-interested/view/' + row.id;
                        var buttonsHtml =
                            // '<a href="' + viewBlogUrl + '" class="btn btn-warning btn-sm mr-1 mb-1" target="_blank"><span class="material-symbols-outlined">visibility</span></a>' +
        ...
        $('#downloadBtn').click(function () {
            ...
            $('#error-download').addClass("hide");
            var downloadUrl = baseUrl + "/admin/job-download?start_date=" + start_date + "&end_date=" + end_date;
    ...
    if ($('#contact-table').length) {
        ...
        $('#downloadBtn').click(function () {
            ...
            $('#error-download').addClass("hide");
            var downloadUrl = baseUrl + "/admin/contact-download?start_date=" + start_date + "&end_date=" + end_date;
            // var downloadUrl = baseUrl + "/admin/contact-download";
    ...
    if ($('#subscribe-table').length) {
        ...
        $('#downloadBtn').click(function () {
            ...
            $('#error-download').addClass("hide");
            var downloadUrl = baseUrl + "/admin/subscribe-download?start_date=" + start_date + "&end_date=" + end_date;
            // var downloadUrl = baseUrl + "/admin/contact-download";
    ...
    if ($('#publication-form-list-table').length) {
        ...
        $('#downloadBtn').click(function () {
            ...
            $('#error-download').addClass("hide");
            var downloadUrl = baseUrl + "/admin/publication-form-list-download?start_date=" + start_date + "&end_date=" + end_date;
            // var downloadUrl = baseUrl + "/admin/contact-download";
            ...
            switch (input.id) {
                ...
                default:
                    if (input.id.startsWith('report_image_')) {
                        ...
                        previewId = input.id.replace('report_image_', 'report_image_preview_');
                    }
                    break;
            }
...
function previewImageEdit(input) {
    ...
    var previewContainer = $(input).closest('.image-upload-div').find('.image-preview-container');
...
jQuery(document).ready(function () {
    ...
    jQuery('.add-report-list-btn').on('click', function () {
        ...
        if (currentCount < maxFields) {
            ...
            Swal.fire('Limit Reached', 'You can only add up to 5 list items.', 'warning');
        }
    });
