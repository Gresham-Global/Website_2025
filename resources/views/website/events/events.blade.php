...
@section('content')
  ...
  <section class="events_main my-5">
    ...
    <div class="events-container" id="eventscrads">
      ...
      <div class="event-title"> <a href="{{ route('events.show', $event->slug) }}" class="event-title">{{ $event->title }} </a></div>
    ...
    <div class="row g-4 addmoreevents text-center {{ $events->count() >= $total ? 'd-none' : '' }} ">
      <button class="mx-auto" id="loadMoreEvents" data-page="2" data-loaded="{{ $events->count() }}" data-total="{{ $total }}">
      <span>Load More</span>
