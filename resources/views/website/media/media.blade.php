...
@section('content')
  ...
  <section class="customSection my-5">
    ...
    <div class="row g-4" id="mediaContainer">
      ...
      <p class="news-card-text flex-grow-1">{{ \Illuminate\Support\Str::limit($item->short_description, 120) }}
      </p>
    ...
    <div class="row g-4 addmoreBlogs text-center {{ $media->count() >= $total ? 'd-none' : '' }}">
      <button id="loadMoreMedia" data-page="2" data-loaded="{{ $media->count() }}" data-total="{{ $total }}">
      <span>Load More</span>
